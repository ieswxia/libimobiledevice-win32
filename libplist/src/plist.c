...
static void plist_get_type_and_value(plist_t node, plist_type * type, void *value, uint64_t * length)
{
    ...
    *length = data->length;
    ...
    case PLIST_DATE:
        ...
        ((timeval_t*) value)->tv_sec = data->timeval.tv_sec;
        ((timeval_t*) value)->tv_usec = data->timeval.tv_usec;
        break;
    case PLIST_ARRAY:
    case PLIST_DICT:
    default:
...
void plist_get_date_val(plist_t node, int32_t * sec, int32_t * usec)
{
    ...
    if (PLIST_DATE == type)
        plist_get_type_and_value(node, &type, (void *) &val, &length);
    assert(length == sizeof(timeval_t));
    *sec = val.tv_sec;
    *usec = val.tv_usec;
}
...
static void plist_set_element_val(plist_t node, plist_type type, const void *value, uint64_t length)
{
    ...
    case PLIST_DATA:
        ...
        break;
    case PLIST_DATE:
        data->timeval.tv_sec = ((timeval_t*) value)->tv_sec;
        data->timeval.tv_usec = ((timeval_t*) value)->tv_usec;
        break;
    case PLIST_ARRAY:
    case PLIST_DICT:
    default:
...
{
    if ( node_n_children((node_t *)node) == 0 )
    {
        plist_data_t data = plist_get_data(node);
        plist_free_data( data );
...
{
    timeval_t val = { sec, usec };
    plist_set_element_val(node, PLIST_DATE, &val, sizeof(timeval_t));
}
